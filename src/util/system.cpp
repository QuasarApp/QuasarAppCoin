...
const char * const BITCOIN_CONF_FILENAME = "quasarappcoin.conf";
...
#else
    const char* pszModule = "quasarappcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\QuasarAppcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\QuasarAppcoin
    // Mac: ~/Library/Application Support/QuasarAppcoin
    // Unix: ~/.quasarappcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "QuasarAppcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/QuasarAppcoin";
#else
    // Unix
    return pathRet / ".quasarappcoin";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure QuasarAppcoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("QuasarAppcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The QuasarAppcoin Core developers";
    }
