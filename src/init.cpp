 ...
 * between bitcoind, and quasarappcoin-qt and non-server tools.
*/
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/quasarappcoin/quasarappcoin>";
...
/** Sanity checks
 *  Ensure that QuasarAppcoin is running in a usable environment with all
...
{
    // Make sure only a single QuasarAppcoin process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if quasarappcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if quasarappcoin is started while in a temporary directory.\n",
